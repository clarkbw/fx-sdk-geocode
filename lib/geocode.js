...
var GeocodeClass = Class({
  ...
  getAddressesByType : function getAddressesByType(type) {
    if (namespace(this).cache.multiple && type in namespace(this).cache.multiple) {
      return namespace(this).cache.multiple[type];
    }
    var result = null;
    if (namespace(this).results) {
      result = namespace(this).results.filter(function (addr) {
        return (addr.types && Array.isArray(addr.types) && addr.types.indexOf(type) >= 0);
      });
    }
    // support a simple caching routine so we don't have to loop
    // through the array on each ask
    if (namespace(this).cache.multiple === null) {
      namespace(this).cache.multiple = {};
    }
    if (result !== null) {
      namespace(this).cache.multiple[type] = result;
    } else {
      result = [];
    }
    return result;
  ...
  getAddressByType : function getAddressByType(type) {
    if (namespace(this).cache.single && type in namespace(this).cache.single) {
      return namespace(this).cache.single[type];
    }
    var result = null;
    if (namespace(this).results) {
    ...
    // support a simple caching routine so we don't have to loop
    // through the array on each ask
    if (namespace(this).cache.single === null) {
      namespace(this).cache.single = {};
    }
    if (result !== null) {
      namespace(this).cache.single[type] = result;
    }
    return result;
  },
